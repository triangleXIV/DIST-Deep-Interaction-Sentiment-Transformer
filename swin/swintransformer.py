...
class PatchMerging(nn.Module):#下采样
...
class SwinTransformerBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.shift_size > 0:  # 选择是否计算偏移窗口
            shifted_x = torch.roll(x, shifts=(-self.shift_size, -self.shift_size), dims=(1, 2))
        else:
            shifted_x = x

        x_windows = window_partition(shifted_x,
                                     self.window_size)  # nW*B, window_size, window_size, C 进行windows划分 将序列转化为窗口
...
class WindowAttention(nn.Module):
    ...
    def forward(self, x, mask=None):
        ...
        x = (attn @ v).transpose(1, 2).reshape(B_, N, C)#将softmax以后的值*V得到结果
        x = self.proj(x)
        x = self.proj_drop(x)
        return x
...
def window_reverse(windows, window_size, H, W):
    #以下是还原操作 让输入和输出的shape相同方便进行下一次计算
