...
class ZOLDatesetReader:
        ...
        with open(fname, 'r', encoding='utf-8', newline='\n', errors='ignore') as fin:
            ...
            for i in range(0, len(lines), 4):
                ...
                if os.path.exists(fname_i):
                    ...
                    logger.info(fname_i)
                    text_raw = lines[i].strip()
                    imgs, num_imgs = self.read_img(lines[i + 1].strip()[1:-1].split(','))# 图像特征提取 这部分不参与训练
                    aspect = lines[i + 2].strip()#获取评价的方面 如续航
                    polarity = int(polarity_dic[(lines[i + 3].strip())]-1)#获取评分 转化为对应分类编号 8分类
    ...
    def __init__(self, dataset='zol_cellphone', max_seq_len=300, max_aspect_len=6, max_img_len=5,img_size=384):
        start = time.time()
